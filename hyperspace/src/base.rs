...
use starlane_space::status::{
    Entity, Status, StatusDetail, StatusProbe, StatusReporter, StatusWatcher,
};
...
impl FoundationBuilder {
    ...
    fn verify(
        &self,
        kind: &ProviderKind,
        verified: &mut HashSet<ProviderKind>,
        stack: &HashSet<ProviderKind>,
    ) -> anyhow::Result<()> {
        ...
        let provider = self
            .providers
            .get(kind)
            .ok_or_else(|| anyhow!("provider kind '{}' not found", kind))?;
        for dep in provider.dependencies() {
            if !verified.contains(&dep) {
                if stack.contains(&dep) {
                    return Err(anyhow!(
                        "provider {} has a circular dependency to {}",
                        kind,
                        dep
                    ));
                }
