...
### BIND
Like the localhost space we need to create a bind for the App to tell it how to process incomming requests.  Here is our bind file:
...
Of course you can see a new type of Pipelie Stop `{{ }}`. This is called an Interal and tells the pipeline to send the request to the Internal portion of the Resource in this case the request will be handled by the Mechtron code.
...
Importanly we have put our route to the app first, since the request will follow the first pattern match that it finds and the pattern for the static site will match any path.

Notice that instead of grabbing the Apps state (which we cannot do since Apps are statelss) we are instead instructing the Pipeline to convert the request to somethig specific
