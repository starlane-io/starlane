...
impl Registry {
    ...
    async fn setup(&self) -> Result<(), sqlx::Error> {
       ...
       CREATE TABLE IF NOT EXISTS tags(
        ...
        let access_grants_index = "CREATE INDEX IF NOT EXISTS query_root_index ON access_grants(query_root)";
        ...
        pub async fn access( &self, to: &Address, on: &Address ) -> Result<Access,Error> {
            ...
            loop {
                ...
                access_grants.retain( |access_grant| {
                    access_grant.to_point.matches(&to_kind_path) && access_grant.on_point.matches(&on_kind_path)
                });
                // check for any superusers
            for access_grant in &access_grants {
                if let Access::Super = self.access(&access_grant.by_particle, &on).await? {
                    match &access_grant.kind {
                        AccessGrantKind::Super => {
                            return Ok(Access::Super)
                        }
...
pub mod test {
    ...
    use std::convert::TryInto;
    use std::str::FromStr;
    use mesh_portal_versions::version::v0_0_1::security::{Access, AccessGrant, AccessGrantKind, Permissions, PermissionsMask, PermissionsMaskKind};
    ...
    pub async fn test_access() -> Result<(), Error> {
        ...
        let grant= AccessGrant{
            kind: AccessGrantKind::PermissionsMask(PermissionsMask::from_str("+rwx-Rwx")?),
            on_point: AddressKindPattern::from_str("localhost:app+:**")?,
        ...
        assert_eq!(access.permissions().child.read, false);
        assert_eq!(access.permissions().child.write, false);
        assert_eq!(access.permissions().child.execute, false);
        assert_eq!(access.permissions().particle.read, false);
        assert_eq!(access.permissions().particle.write, false);
        assert_eq!(access.permissions().particle.execute, false);
        ...
        assert_eq!(access.permissions().child.write, false);
        assert_eq!(access.permissions().child.execute, false);
        assert_eq!(access.permissions().particle.read, true);
        assert_eq!(access.permissions().particle.write, false);
        assert_eq!(access.permissions().particle.execute, false);
        ...
        assert_eq!(access.permissions().particle.read, false );
        assert_eq!(access.permissions().particle.write, false);
        assert_eq!(access.permissions().particle.execute, true );
