...
use mesh_portal_versions::version::v0_0_1::wave::{AsyncInternalRequestHandlers, AsyncPointRequestHandlers, AsyncRequestHandler, AsyncRequestHandlerRelay, AsyncRouter, AsyncTransmitter, AsyncTransmitterWithAgent, ReqCtx, ReqXtra, Requestable, RespXtra, Wave, WaveXtra, RequestHandler};
use std::sync::Arc;
...
impl Shell {
    pub async fn new(
        ...
        mut inlet_rx: mpsc::Receiver<WaveXtra>,
        core_tx: mpsc::Sender<WaveXtra>,
        logger: RootLogger,
    ) -> Self {
                    ...
                    match frame {
                        WaveXtra::Req(frame) => {
                            ...
                            if frame.to().point == port.point {
                                ...
                                let frame: RespXtra =
                                    stub.result(core_messenger.send(frame.request.into()).await);
                                let frame: WaveXtra = frame.into();
                                core_tx.send(frame).await;
                            }
                        }
        ...
        let cli = Cli::new(
            transmitter
                .clone()
                .with_from(port.clone().with_layer(Layer::Core)),
        );
        let cli_relay = CliRelay::new(port.clone(), transmitter.clone());
        handlers.add(
            RouteSelector::any(),
            AsyncRequestHandlerRelay::new(Box::new(cli_relay)),
        );
