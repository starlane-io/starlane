...
pub mod test {
    ...
    use crate::parse::{
        args, assignment, base_point_segment, base_seg, command_line, comment, consume_point_var,
        create, create_command, doc, expected_block_terminator_or_non_terminator, lex_block,
...
Mechtron(version=1.0.0) {
    ...
    pub fn test_bind_config() -> Result<(), SpaceErr> {
        ...
        let bind_config_str = r#"
        Bind(version=1.0.0)
        {
            Route -> {
            Http<Get>/profiles/(?P<user>.*$) -> .:profiles:${user} => &;
        }
        }
           "#;

        assert!(util::log(doc(bind_config_str)).is_ok());
        Ok(())
    }
