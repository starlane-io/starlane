...
use crate::driver::{Driver, DriverCtx, DriverSkel, HyperDriverFactory, ItemDirectedHandler, ItemHandler};
...
impl <P> Driver<P> for BaseDriver where P: Platform {
    ...
    async fn item(&self, point: &Point) -> Result<ItemHandler<P>, P::Err> {
        Ok(ItemHandler::Handler(Box::new(Base)))
    }
}
...
#[async_trait]
impl <P> ItemDirectedHandler<P> for Base where P: Platform {
