...
use crate::star::{Star, StarApi, StarSkel, StarTemplate};
...
pub struct Machine {
   ...
   pub stars: HashMap<StarKey,StarApi>,
   pub lanes: HashMap<StarKey,HyperwayInterchange>,
...
impl Machine {
    pub fn new( skel: MachineSkel, implementation: Box<dyn Implementation>, template: MachineTemplate ) -> Result<(),MsgErr> {
        ...
        for star_template in template.stars {
            ...
            let (fabric_tx, fabric_rx) = mpsc::channel(32*1024);
            ...
            stars.insert(star_skel.key.clone(), star_api );
