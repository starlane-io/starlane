...
fn get_artifacts(data: Arc<Vec<u8>>) -> Result<Vec<String>, Error> {
    ...
    match archive {
        ...
        Err(_error) => Err("ArtifactBundle must be a properly formatted Zip file.".into()),
    }
}
        ...
        if let Substance::Bin(zip) = (*state).clone() {
            ...
            for artifact in artifacts {
                ...
                for (index, segment) in segments.iter().enumerate() {
                    ...
                    let point_and_kind = AddressAndKind { point, kind };
            ...
            let mut point_and_kind_set: Vec<AddressAndKind> =
        ...
        if let Kind::Artifact(artifact_kind) = kind {
            match artifact_kind {
                ...
                _ => {
                    ...
                    self.store
                        .put(assign.details.stub.point.clone(), *state)
                        .await?;
                    Ok(())
                }
            }
