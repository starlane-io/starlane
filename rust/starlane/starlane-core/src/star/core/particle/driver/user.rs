...
impl ParticleCoreDriver for UserBaseKeycloakCoreDriver {
    ...
    async fn assign(&mut self, assign: Assign) -> Result<(), Error> {
        ...
        if is_hyper_userbase(&assign.details.stub.point) {
            let sso_session_max_lifespan = assign
                .details
                .properties
                .get("sso-session-max-lifespan")
                .ok_or("cannot get value for required property 'sso-session-max-lifespan'")?
                .value
                .clone();
            self.admin
                .init_realm_for_point("master".to_string(), &assign.details.stub.point)
                .await?;
            match self
                .admin
                .update_realm_for_point(
                    "master".to_string(),
                ...
                .await
            {
                Err(err) => {
                    error!("{}", err.to_string());
                    return Err(format!(
                        "UserBase<Keyloak>: could not update master realm for {}",
            ...
            let registration_email_as_username = assign
                .details
                .properties
                .get("registration-email-as-username")
                .map_or(None, |x| Some(x.value == "true"));
            let verify_email = assign
                .details
                .properties
                .get("verify-email")
                .map_or(None, |x| Some(x.value == "true"));
                ...
                .await
            {
                Err(err) => {
                    error!("{}", err.to_string());
                    return Err(format!(
                        "UserBase<Keyloak>: could not create realm for {}",
                        assign.details.stub.point.to_string()
                    )
                    .into());
                }
    ...
    async fn handle_request(&self, request: ReqShell) -> RespShell {
        ...
        match &request.core.method {
            ...
            Method::Msg(_) => self.handle_msg(request).await,
        }
    }
    ...
    async fn particle_command(&self, to: Point, command: Command) -> Result<Substance, Error> {
        match command {
            ...
            _ => Err("UserBase does not handle this type of command".into()),
        }
    }
}
...
impl UserBaseKeycloakCoreDriver {
    fn keycloak_url() -> Result<String, Error> {
        Ok(std::env::var("STARLANE_KEYCLOAK_URL")
    ...
    async fn handle_msg(&self, request: ReqShell) -> RespShell {
        if let Method::Msg(method) = &request.core.method {
            println!(
                "UserBase<Keycloak> ... Handle Message action: {}",
                method.to_string()
            );
            match method.as_str() {
                "GetJwks" => request
                    .clone()
                    .body_result(self.handle_get_jwks(&request).await),
    ...
    async fn handle_http(&self, request: ReqShell) -> RespShell {
        ...
        if let Method::Http(method) = &request.core.method {
            match method {
                &HttpMethod::Post => match &request.core.uri.path() {
                    &"/login" => request.clone().result(self.handle_login(&request).await),
                    &"/introspect" => request
                        .clone()
                        .body_result(self.handle_introspect_token(&request).await),
                    &"/refresh-token" => request
                        .clone()
                        .body_result(self.handle_refresh_token(&request).await),
                    _ => request.status(404),
                },
                _ => request.status(404),
            }
    ...
    async fn handle_login(&self, request: &ReqShell) -> Result<RespCore, Error> {
        let multipart: Vec<(String, String)> =
            serde_urlencoded::from_bytes(request.core.body.clone().to_bin()?.as_bytes())?;
    ...
    async fn handle_introspect_token(&self, request: &ReqShell) -> Result<Substance, Error> {
        ...
        let jwk = jwks
            .find(&kid)
            .ok_or(format!("expected to find kid: {}", kid))?;
    ...
    async fn set_child(&self, to: Point, mut set: Set) -> Result<Substance, Error> {
        ...
        let email = record
            ...
            .properties
            .get("email")
            .ok_or("User missing 'email' property")?
            .value
            .clone();

        if password.is_some() {
            self.admin
                .reset_password(&to, email, password.unwrap())
                .await?;
        }
            ...
            PointSegFactory::Exact(username) => {
                if realm.registration_email_as_username.unwrap_or(false) {
                    ...
                    if !is_hyperuser(&stub.point) {
                        ...
                        attributes
                            .insert("point".to_string(), Value::String(stub.point.to_string()));
                        self.admin
                            .add_user_attributes(
                                &create.template.point.parent,
                                username.to_string(),
...
impl StarlaneKeycloakAdmin {
    ...
    pub async fn init_realm_for_point(
        ...
        let clients = self
            .admin
            .realm_clients_get(realm.clone().as_str(), None, None, None, None, None)
            .await?;
    ...
    pub async fn select_all(
        ...
        first: i32,
        max: i32,
    ) -> Result<Vec<UserRepresentation>, Error> {
        let realm = normalize_realm(realm);
        Ok(self
            .admin
            .realm_users_get(
