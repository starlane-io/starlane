...
use crate::frame::{
    Frame, ProtoFrame, ResourceRegistryRequest, SimpleReply, StarMessage, StarMessagePayload,
    WatchFrame,
};
            ...
            if message.to == skel.info.key {
                    ...
                    &message.payload
                {
                    if let RouteSegment::Fabric(_) = &point.route {
                        ...
                        match result {
                            Ok(record) => {
                                let reply = message.reply(StarMessagePayload::Reply(
                                    SimpleReply::Ok(Reply::Record(record)),
                                ));
                                skel.messaging_api.star_notify(reply);
                            }
                            Err(err) => {
                                let reply = message.reply(StarMessagePayload::Reply(
                                    SimpleReply::Fail(Fail::Starlane(StarlaneFailure::Error(
                                        "Not Found".to_string(),
                                    ))),
                                ));
                                skel.messaging_api.star_notify(reply);
                        ...
                        let result = skel.registry_api.locate(&point).await;
                        match result {
                            Ok(record) => {
                                let reply = message.reply(StarMessagePayload::Reply(
                                    SimpleReply::Ok(Reply::Record(record)),
                ...
                if let Result::Ok(lane) = skel
                    .golden_path_api
                    .golden_lane_leading_to_star(message.to.clone())
                    .await
                {
                    skel.lane_muxer_api
    ...
    async fn frame(&self, frame: Frame, session: LaneSession) {
        ...
        if let FrameVerdict::Handle(frame) = verdict {
            match frame {
                Frame::Proto(_) => {}
                Frame::Diagnose(_) => {}
                Frame::SearchTraversal(traversal) => match &session.lane {
                    LaneKey::Proto(_) => {
                    ...
                    LaneKey::Ultima(lane) => {
                        self.skel
                            .star_search_api
                            .on_traversal(traversal, lane.clone());
                    }
                },
                Frame::StarMessage(message) => {
                    self.route(message);
                }
                Frame::Watch(watch) => match watch {
