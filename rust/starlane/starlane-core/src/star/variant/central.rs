    ...
    fn init_variant(&mut self, tx: oneshot::Sender<Result<(), Error>>) {
        if self.initialized == true {
            ...
            tokio::spawn(async move {
                match skel
                    .machine
                    .get_starlane_api(HyperUser::point().to_port())
                    .await
                {
                    Ok(api) => {
                    ...
                    Err(err) => {
                        error!(
                            "could not get starlane api for central init: {}",
                            err.to_string()
                        );
                        tx.send(Err(err));
                    }
...
impl CentralVariant {
    async fn ensure(starlane_api: StarlaneApi) -> Result<(), Error> {
        ...
        session
            .exec("create? hyperspace:repo:boot<ArtifactBundleSeries>")
            .await;
        let content = Arc::new(BOOT_BUNDLE_ZIP.to_vec());
        session
            .exec_with_transfers(
                "publish? ^[ bundle.zip ]-> hyperspace:repo:boot:1.0.0",
                vec![Transfer::new("bundle.zip", content)],
            )
            .await;
        session
            .exec("create? hyperspace:users<UserBase<Keycloak>>")
            .await;
        session
            .exec("create? hyperspace:users:hyperuser<User>")
            .await;
