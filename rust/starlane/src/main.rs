...
fn main() -> Result<(), Error> {
    ...
    if let Option::Some(serve) = matches.subcommand_matches("serve") {
        ...
        shutdown();
    } else if let Option::Some(args) = matches.subcommand_matches("xp") {
let rt = Runtime::new().unwrap();
rt.block_on(async {
xp().await.unwrap();
});
...
async fn xp() -> Result<(), Error> {
    let starlane_api = starlane_api().await?;

    xp2(starlane_api.clone(),"hyperspace:starlane:fs").await?;
    xp2(starlane_api.clone(),"hyperspace:starlane:fs2").await?;
    Ok(())
}

async fn xp2(starlane_api: StarlaneApi,  dst: &str ) -> Result<(), Error> {
println!("DST: {}", dst );
    if ResourcePath::from_str(dst ).is_ok() {
        let dst = ResourcePath::from_str(dst)?;
        // copying from src to dst
        ...
        let create = ResourceCreate {
            ...
            state_src: AssignResourceStateSrc::Stateless,
            registry_info: Option::None,
            owner: Option::None,
            strategy: ResourceCreateStrategy::CreateOrUpdate,
        };
