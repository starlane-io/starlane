...
impl Postgres {
    ...
    fn embedded_postgresql(config: &StarlaneConfig) -> Result<Settings,RegErr> {
    ...
    pub async fn install(config: &StarlaneConfig ) -> Result<(), RegErr> {
    ...
    pub async fn new(config: &StarlaneConfig) -> Result<Self, RegErr> {
        let mut postgres = PostgreSQL::new(Self::embedded_postgresql(config)?);

        let config = config.clone().registry.try_into()?;
