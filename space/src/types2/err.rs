...
pub enum TypeErr {
    ...
    EmptyMeta(Type),
    #[error(
        "{kind} Meta::by_layer({tried}) index out of bounds because exceeds layers length {len}"
    )]
...
impl TypeErr {
    ...
    pub fn specific_not_found(specific: Slice, search_location: String) -> Self {
        Self::SpecificNotFound {
            search_location,
            specific,
        }
    }
