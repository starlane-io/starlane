...
impl PropertyPattern for UsernamePattern {
    fn is_match(&self, value: &String) -> Result<(), SpaceErr> {
        PropertyName::from_str(value.as_str())?;
        Ok(())
    }
}
...
impl PropertiesConfig {
    ...
    pub fn check_create(&self, set: &SetProperties) -> Result<(), SpaceErr> {
        ...
        for (key, prop_mod) in &set.map {
            let def = self.get(key.as_str()).ok_or(format!(
                "{} illegal property: '{}'",
                self.kind.to_string(),
                key
            ))?;
...
impl SetProperties {
    ...
    pub fn iter(&self) -> Iter<'_, PropertyName, PropertyMod> {
        self.map.iter()
    }
}

impl Deref for SetProperties {
    type Target = HashMap<PropertyName, PropertyMod>;
...
pub type PropertyName = SkewerCase;
